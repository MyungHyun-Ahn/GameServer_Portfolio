...
DROP PROCEDURE IF EXISTS sp_ad_u_bid_revert;
...
DROP PROCEDURE IF EXISTS sp_ad_u_expire_revert;
...
CREATE PROCEDURE sp_ad_d_listing_pending
(
    IN p_listing_id BIGINT UNSIGNED,
    IN p_seller_user_id BIGINT UNSIGNED,
    IN p_expected_version BIGINT UNSIGNED
...
(
    ...
    IN p_minimum_bid_increment BIGINT UNSIGNED
)
BEGIN
    ...
    DECLARE v_bid_id BIGINT UNSIGNED DEFAULT 0;
    DECLARE v_listing_found BOOLEAN DEFAULT TRUE;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET v_listing_found = FALSE;

    SELECT seller_user_id, currency_id, start_price, current_bid_price, buyout_price,
           highest_bid_id, highest_bidder_user_id, version
      INTO v_seller_user_id, v_currency_id, v_start_price, v_current_bid_price, v_buyout_price,
    ...
    END IF;
    IF v_buyout_price IS NOT NULL AND p_bid_amount >= v_buyout_price THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BID_REQUIRES_BUYOUT';
    END IF;
    IF v_highest_bid_id IS NULL THEN
        IF p_bid_amount < v_start_price THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BID_TOO_LOW';
        END IF;
    ELSE
        IF p_bid_amount <= v_current_bid_price THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BID_TOO_LOW';
        ELSEIF p_bid_amount - v_current_bid_price < p_minimum_bid_increment THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BID_TOO_LOW';
        END IF;
    END IF;
...
BEGIN
    DECLARE v_listing_found BOOLEAN DEFAULT TRUE;
    DECLARE v_currency_id SMALLINT UNSIGNED;
    DECLARE v_previous_highest_bid_id BIGINT UNSIGNED;
    DECLARE v_existing_bid_id BIGINT UNSIGNED;
    DECLARE v_existing_bid_state TINYINT UNSIGNED;
    DECLARE v_existing_bid_version BIGINT UNSIGNED;
    ...
    SELECT currency_id, highest_bid_id
      INTO v_currency_id, v_previous_highest_bid_id
      FROM auction_listings
     WHERE listing_id = p_listing_id
       AND state = 3
    ...
    SELECT MAX(bid_id), MAX(state), MAX(version)
      INTO v_existing_bid_id, v_existing_bid_state, v_existing_bid_version
      FROM auction_bids
     WHERE listing_id = p_listing_id
       AND bidder_user_id = p_bidder_user_id;
    ...
    IF v_existing_bid_id IS NULL THEN
        ...
        VALUES(p_listing_id, p_bidder_user_id, v_currency_id, p_bid_amount, 2, 1);
        SET v_bid_id = LAST_INSERT_ID();
    ELSE
        IF v_existing_bid_state NOT IN (2, 3, 5) THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BID_COMPLETE_FAILED';
        END IF;
        UPDATE auction_bids
           SET bid_amount = p_bid_amount, state = 2, version = version + 1
    ...
    IF v_previous_highest_bid_id IS NOT NULL AND v_previous_highest_bid_id <> v_bid_id THEN
        UPDATE auction_bids
           SET state = 3, version = version + 1
         WHERE bid_id = v_previous_highest_bid_id AND state = 2;
        IF ROW_COUNT() <> 1 THEN
    ...
    SELECT v_bid_id AS bid_id, version AS listing_version
      FROM auction_listings
     WHERE listing_id = p_listing_id;
END$$

CREATE PROCEDURE sp_ad_u_bid_revert
(
    IN p_listing_id BIGINT UNSIGNED,
    IN p_expected_listing_version BIGINT UNSIGNED
)
BEGIN
    UPDATE auction_listings
     ...
     WHERE listing_id = p_listing_id
       ...
       AND version = p_expected_listing_version;
    IF ROW_COUNT() <> 1 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BID_REVERT_FAILED';
    END IF;
END$$
...
BEGIN
    ...
    DECLARE v_buyer_bid_id BIGINT UNSIGNED;
    DECLARE v_buyer_bid_state TINYINT UNSIGNED;
    DECLARE v_buyer_bid_version BIGINT UNSIGNED;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET v_found = FALSE;

    SELECT buyout_price, highest_bid_id
      INTO v_buyout_price, v_highest_bid_id
      FROM auction_listings
     WHERE listing_id = p_listing_id
       AND state = 4
       AND version = p_expected_listing_version
     FOR UPDATE;
    IF NOT v_found OR v_buyout_price IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BUYOUT_COMPLETE_FAILED';
    END IF;

    SELECT MAX(bid_id), MAX(state), MAX(version)
      INTO v_buyer_bid_id, v_buyer_bid_state, v_buyer_bid_version
      FROM auction_bids
     WHERE listing_id = p_listing_id
       AND bidder_user_id = p_buyer_user_id;

    IF v_buyer_bid_id IS NOT NULL AND v_buyer_bid_state IN (2, 3) THEN
        UPDATE auction_bids
           SET state = 6, version = version + 1
         WHERE bid_id = v_buyer_bid_id AND version = v_buyer_bid_version;
        IF ROW_COUNT() <> 1 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BUYOUT_COMPLETE_FAILED';
        END IF;
    END IF;
...
CREATE PROCEDURE sp_ad_u_buyout_revert
(
    IN p_listing_id BIGINT UNSIGNED,
    IN p_expected_listing_version BIGINT UNSIGNED
)
BEGIN
    UPDATE auction_listings
     ...
     WHERE listing_id = p_listing_id
       ...
       AND version = p_expected_listing_version;
    IF ROW_COUNT() <> 1 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BUYOUT_REVERT_FAILED';
    END IF;
END$$
...
CREATE PROCEDURE sp_ad_u_cancel_revert
(
    IN p_listing_id BIGINT UNSIGNED,
    IN p_seller_user_id BIGINT UNSIGNED,
    IN p_expected_listing_version BIGINT UNSIGNED
...
BEGIN
    UPDATE auction_listings SET state=2,version=version+1
     WHERE listing_id=p_listing_id AND seller_user_id=p_seller_user_id
       AND state=5 AND version=p_expected_listing_version;
    IF ROW_COUNT() <> 1 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'CANCEL_REVERT_FAILED';
    END IF;
END$$
...
BEGIN
    ...
    SELECT highest_bid_id, highest_bidder_user_id
      INTO v_highest_bid_id, v_highest_bidder_user_id
      FROM auction_listings
    ...
    IF p_winner_user_id=0 THEN
        IF v_highest_bid_id IS NOT NULL OR v_highest_bidder_user_id IS NOT NULL OR p_final_price<>0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='EXPIRE_COMPLETE_FAILED';
        END IF;
        UPDATE auction_listings
         ...
         WHERE listing_id=p_listing_id AND state=6 AND version=p_expected_listing_version;
    ELSE
        IF v_highest_bid_id IS NULL OR v_highest_bidder_user_id<>p_winner_user_id THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='EXPIRE_COMPLETE_FAILED';
        END IF;
        UPDATE auction_bids SET state=6,version=version+1
         WHERE bid_id=v_highest_bid_id AND bidder_user_id=p_winner_user_id AND state=2;
        IF ROW_COUNT()<>1 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='HIGHEST_BID_STATE_INVALID';
        END IF;
        UPDATE auction_listings
...
CREATE PROCEDURE sp_ad_u_expire_revert
(
    IN p_listing_id BIGINT UNSIGNED,
    IN p_expected_listing_version BIGINT UNSIGNED
)
BEGIN
    UPDATE auction_listings SET state=2,version=version+1
     WHERE listing_id=p_listing_id AND state=6 AND version=p_expected_listing_version;
    IF ROW_COUNT()<>1 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='EXPIRE_REVERT_FAILED';
    END IF;
END$$
