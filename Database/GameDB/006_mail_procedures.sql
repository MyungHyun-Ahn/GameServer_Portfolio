...
(
    ...
    IN p_item_data JSON,
    IN p_mail_type INT UNSIGNED,
    IN p_subject VARCHAR(200),
    IN p_body TEXT,
    IN p_expiration_seconds INT UNSIGNED
)
BEGIN
    ...
    IF p_receiver_user_id = 0 OR p_item_instance_id = 0 OR p_item_data_id = 0 OR
       p_quantity = 0 OR p_item_data IS NULL OR p_mail_type = 0 OR p_mail_type > 255 OR
       p_subject IS NULL OR CHAR_LENGTH(TRIM(p_subject)) = 0 OR CHAR_LENGTH(p_subject) > 200 OR
       p_body IS NULL OR CHAR_LENGTH(TRIM(p_body)) = 0 OR CHAR_LENGTH(p_body) > 65535 OR
       p_expiration_seconds = 0 THEN
    ...
    VALUES(p_receiver_user_id,p_mail_type,p_subject,p_body,1,
           DATE_ADD(UTC_TIMESTAMP(6),INTERVAL p_expiration_seconds SECOND));
...
BEGIN
    DECLARE v_mail_id BIGINT UNSIGNED;
    DECLARE v_attachment_id BIGINT UNSIGNED;
    IF p_receiver_user_id = 0 OR p_currency_id = 0 OR p_currency_amount = 0 OR
       p_mail_type = 0 OR p_mail_type > 255 OR
       p_subject IS NULL OR CHAR_LENGTH(TRIM(p_subject)) = 0 OR CHAR_LENGTH(p_subject) > 200 OR
       p_body IS NULL OR CHAR_LENGTH(TRIM(p_body)) = 0 OR CHAR_LENGTH(p_body) > 65535 OR
       p_expiration_seconds = 0 THEN
...
(
    ...
    IN p_mail_type INT UNSIGNED,IN p_subject VARCHAR(200),IN p_body TEXT,
    IN p_expiration_seconds INT UNSIGNED
)
BEGIN
    DECLARE v_mail_id BIGINT UNSIGNED;
...
(
	...
	IN p_item_is_tradable TINYINT UNSIGNED,
...
BEGIN
	...
	DECLARE v_currency_version BIGINT UNSIGNED DEFAULT 0;
    ...
    IF v_attachment_type = 1 THEN
		IF p_item_is_tradable NOT IN (0, 1) THEN
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'INVALID_ITEM_TRADABLE';
		END IF;
        ...
        VALUES(v_item_instance_id,p_receiver_user_id,v_item_data_id,v_quantity,v_item_data,0,p_item_is_tradable,1);
    ELSEIF v_attachment_type = 2 THEN
        ...
        END IF;
		SELECT COUNT(*),COALESCE(MAX(amount),0),COALESCE(MAX(version),0)
		  INTO v_wallet_exists,v_balance,v_currency_version
        ...
        ELSE
			...
			SET v_currency_version = v_currency_version + 1;
        END IF;
    ELSE
