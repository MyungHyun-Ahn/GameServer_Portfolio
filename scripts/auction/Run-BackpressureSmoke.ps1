...
{
    throw "Auction test executables are missing. Build the projects first."
}

$testDirectory = Join-Path $repositoryRoot ("Out\AuctionBackpressureTest\" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
New-ServerConfigFile `
    -TemplatePath $serverConfigTemplate `
    -DestinationPath $serverConfig `
    -Overrides @{
        "AuctionHouseServer.Port" = $Port
        "AuctionHouseServer.RunSeconds" = 8
        "AuctionHouseServer.CommandMailboxCapacity" = 4
        ...
        "Debug.TestDelayShardIndex" = 0
        "Debug.TestDelayMilliseconds" = 200
    } | Out-Null
...
try
{
    ...
    & $clientExecutable --port $Port --backpressure-test 1> $clientStandardOutput 2> $clientStandardError
    $clientExitCode = $LASTEXITCODE

    if (-not $serverProcess.WaitForExit(12000))
    {
        throw "AuctionHouseServer did not exit within the timeout."
    }

    $clientOutput = Get-Content -LiteralPath $clientStandardOutput -Raw
    if ($clientExitCode -ne 0 -or $clientOutput -notmatch "BACKPRESSURE_TEST_SUCCESS")
    {
        $clientError = Get-Content -LiteralPath $clientStandardError -Raw -ErrorAction SilentlyContinue
        throw "Backpressure smoke test failed. exit=$clientExitCode error=$clientError"
    }
