...
namespace Generated
{
	...
	public:
		...
		void SetMapHandler(
			Map::IMapPacketDispatcher* handler) noexcept
		{
			m_mapHandler = handler;
		}

		void SetWorldHandler(
			World::IWorldPacketDispatcher* handler) noexcept
		{
			m_worldHandler = handler;
		}
			...
			const NetworkLib::Packet::View::FPacketView& packetView)
		{
			switch (packetView.opcode)
			{
				...
				case Login::FLoginAuthRp::kOpcode:
					return m_loginHandler != nullptr ? m_loginHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FMapEnterRq::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FMapEnterRp::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FActorSpawnNoti::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FActorDespawnNoti::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FMoveRq::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FMoveRp::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FMoveNoti::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FActorAttackNoti::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FActorDeathNoti::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FActorRespawnNoti::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FBasicAttackRq::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case Map::FBasicAttackRp::kOpcode:
					return m_mapHandler != nullptr ? m_mapHandler->DispatchPacket(server, sessionId, packetView) : false;
				case World::FWorldAuthRq::kOpcode:
					return m_worldHandler != nullptr ? m_worldHandler->DispatchPacket(server, sessionId, packetView) : false;
				case World::FWorldAuthRp::kOpcode:
					return m_worldHandler != nullptr ? m_worldHandler->DispatchPacket(server, sessionId, packetView) : false;
				case World::FEquipItemRq::kOpcode:
					return m_worldHandler != nullptr ? m_worldHandler->DispatchPacket(server, sessionId, packetView) : false;
				case World::FEquipItemRp::kOpcode:
					return m_worldHandler != nullptr ? m_worldHandler->DispatchPacket(server, sessionId, packetView) : false;
				case World::FUnequipItemRq::kOpcode:
					return m_worldHandler != nullptr ? m_worldHandler->DispatchPacket(server, sessionId, packetView) : false;
				case World::FUnequipItemRp::kOpcode:
					return m_worldHandler != nullptr ? m_worldHandler->DispatchPacket(server, sessionId, packetView) : false;
				default:
					return false;
			}
	...
	private:
		...
		Map::IMapPacketDispatcher* m_mapHandler = nullptr;
		World::IWorldPacketDispatcher* m_worldHandler = nullptr;
	};
}
