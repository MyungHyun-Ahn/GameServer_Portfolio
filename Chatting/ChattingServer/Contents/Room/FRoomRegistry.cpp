...
namespace ChattingServer::Contents
{
	...
	std::vector<std::uint64_t> FRoomRegistry::GetSessionIdsInRoom(
		const std::uint32_t roomId) const
	{
		std::lock_guard<std::mutex> lock(m_lock);
		...
		if (roomIt != m_rooms.end())
		{
			sessionIds.reserve(roomIt->second.snapshot.participantCount);
		}

		for (const auto& [sessionId, sessionState] : m_sessionRooms)
		{
			if (sessionState.roomId == roomId && sessionState.activeMember)
			{
		...
		ContentsRuntime::Core::FContentInstanceId& outContentInstanceId)
	{
		...
		m_sessionRooms[sessionId] = SSessionRoomState{roomId, targetRouteGeneration, false};
		...
		ContentsRuntime::Core::FContentInstanceId& outContentInstanceId)
	{
		...
		sessionIt->second = SSessionRoomState{targetRoomId, targetRouteGeneration, false};
		outContentInstanceId = targetRoomIt->second.snapshot.contentInstanceId;
		return ERoomFlowResultCode::Success;
	}

	bool FRoomRegistry::ActivateSessionRoom(
		const std::uint64_t sessionId,
		const std::uint32_t roomId,
		const std::uint64_t routeGeneration)
	{
		...
		sessionIt->second.activeMember = true;
		return true;
	}
		...
		const std::uint64_t targetRouteGeneration)
	{
		...
		sessionIt->second = SSessionRoomState{previousRoomId, targetRouteGeneration - 1, true};
	}
